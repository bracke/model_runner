...
take **0.455 s** -- 0.069 s evaluating the prompt and 0.385 s generating --
...
f32`, taken back to back in the same sitting, is **1.419 s** for 8.86 s of
...
which is not the same thing. Eight shares take 0.455 s for 1.61 s of processor
time and fifteen take 0.486 s for 1.82 s: the second worker on a core shares
the first one's execution units, and what is left over for it to use has not
only gone but turned negative. Seven per cent *slower* for thirteen per cent
...
load of 1.05 throughout: `cpu` spends 0.069 s evaluating the prompt and
0.126 s generating; `reference` spends 5.800 s and 3.880 s. That is **fifty
...
| --- | --- | --- |
| 6-token prompt, 12 generated | 0.455 s | **0.428 s** |
| -- evaluating the prompt | 0.067 s | 0.051 s |
| -- generating | 0.383 s | 0.377 s |
| -- processor time | 1.61 s | **0.04 s** |
| 110-token prompt, nothing generated | 0.719 s | **0.537 s** |
| -- processor time | 3.55 s | **0.11 s** |
...
0.537 s -- the default batch, the results a product reads back, the
...
| --- | ---: | ---: |
| model_runner, processor | 153.0 t/s | 29.3 t/s |
| llama.cpp, processor | 380.9 t/s | 40.0 t/s |
| model_runner, device | 204.8 t/s | 27.1 t/s |
| llama.cpp, device | 1613.2 t/s | 57.5 t/s |
...
speed` reports seconds and this table reports rates: 110 tokens in 0.719 s
and 64 in 2.187 s on the processor, 0.537 s and 2.360 s on the device,
...
device present llama.cpp still evaluates the prompt on it -- 868.3 t/s rather
than 380.9 -- so a reader who takes this again the obvious way will measure
...
the slow one: 27.1 t/s here, against 31.0, 30.9, 27.3, 26.9, 31.0, 31.2, 28.1, 31.8, 32.0, 31.1, 30.7, 30.5, 22.0, 21.1, 23.3, 24.2, 18.2, 15.9, 17.7, 14.9, 14.1, 14.1, 13.7, 16.9, 16.2 and 13.3 in ten
...
140.3 to 140.7 to 136.5 to 149.1 to 153.0 on the prompt, and 11.0 to 12.2 to about 23 to 28.6 to 29.0 to 27.9 to 27.6 to
...
| | Twelve tokens | |
| --- | --- | --- |
| TinyLlama-1.1B at eight bits | 0.457 s | 38 ms a token |
| the same model at two bits | 1.777 s | 148 ms a token |
| the first, drafted by the second | 3.870 s | 24 proposed, 9 accepted |
...
cost 3.870 s, of which the draft's own twenty-four passes are 24 × 148 ms =
3.55 s, leaving 0.32 s for six checks -- **53 ms to check five positions**,
...
So a round of K proposals costs `K × d + 53 ms` and yields `1 + a` tokens,
...
costs 53: a draft would have to cost under about 10 ms a token to pay, and
the one here costs 148. This figure has read 53, 72 and 120 ms across three
sittings of the same code, because it is a difference of two twelve-token
runs and a twelve-token run is a third loading and warm-up. Read it as tens
of milliseconds and not as a number. The check got a great
...
Medians of three alternated rounds, the same digest again, and generating
unmoved because a six-bit product of one vector still goes the other way.
**Four and a half times, end to end**: this file read a 110-token prompt in
4.598 s this morning.
...
host admits, which on this one is the byte dot product, and it reads **4.88
...
| | ms a token | share |
|---|---:|---:|
| feed forward, gate up down | 9.712 | 60.6 % |
| attention projections, q k v o | 3.674 | 22.9 % |
| the vocabulary projection | 1.545 | 9.6 % |
| activation and gate | 0.655 | 4.1 % |
| softmax over the vocabulary | 0.173 | 1.1 % |
| normalization, twice a layer | 0.159 | 1.0 % |
| rotation | 0.102 | 0.6 % |
| normalization, once at the end | 0.004 | 0.0 % |
| **what a token costs of these** | **16.024** | |

The same token at `--arith f32` costs 77.253 ms of these, so the products are
**4.8 times** what they were and everything else is where it was. Reading a
prompt, thirty-two a pass, the whole is 5.527 ms a token in the same shape --
...
|---|---:|---:|
| feeding | 0.474 | 67.3 % |
| projecting | 0.141 | 19.9 % |
| attending | 0.037 | 5.2 % |
| joining | 0.022 | 3.1 % |
| normalizing | 0.018 | 2.5 % |
| rotating | 0.012 | 1.7 % |
| reading out | 0.002 | 0.3 % |
| **accounted for** | **0.705** | |

**The run this came from accounts for 0.705 s of itself**, which is the
...
**The products are 87.2 per cent of a prompt, and the feed-forward is 3.4

...
both walks over the vocabulary -- the one looking for a logit that is not a
number and the one looking for the highest -- divide across whatever workers
the caller has, into blocks that do not move when the team does, so the token
is the same at every worker count and with none; xoshiro256++ seeded per session. `--logit-bias TOKEN=X` nudges a token; `--logprobs N` reports what the model made of each position, from a plain softmax over the raw logits with none of the sampling applied |
...
**Work crosses the layering downwards through one pair of interfaces rather
than upwards through a dependency.** `Model_Runner.Shares` declares `Work` --
something a range of items can be asked of -- and `Team` -- something that
can run the pieces of a job at the same time. The worker pool in the CPU
backend wears `Team`; the engine's shares and the sampler's wear `Work`. That
is how a package which decides what a model says gets shared out without
naming a package that asks the platform what it has and creates tasks. What a
team may be handed is bounded by one rule, stated there: the answer must not
...
That pair was taken in one sitting for the contrast and is kept as it was;
the same four rows in this sitting, on a part that had been working, read
2503, 2486, 2395 and 13066.
...
| --- | ---: | --- | ---: | --- | ---: | --- |
| one vector | 13342/12952 | +3 % | 13610/13550 | +0.4 % | 13407/13066 | +3 % |
| thirty-two a pass | 22897/20754 | +10 % | 22804/23039 | −1 % | 21648/21597 | +0.2 % |
| one share, one vector | 2385/2417 | −1 % | 2473/2509 | −1 % | 2474/2395 | +3 % |

The four-bit column barely moves across all three; the eight-bit batched
figure went 20754, 23039, 21597 on code that changed once. A row that swings
...
Comparing the two at `--arith f32` gives eighteen times, and the ratio at
...
serial against serial, no pool on either side -- and reports 2.39x for q8_0,
2.37x for q4_k and 3.19x for f32. The rest of the eighteen is the worker pool
...
It is the row `### A prompt too short to wake the machine` below is about: a
tenth of a second of work never brings the part above about 1.5 GHz, and
where inside its low band the clock happens to sit decides the cell. The
middle sitting was the odd one -- alternated binaries read 0.123 and 0.122
there and 0.100 here -- and the lesson is the one this file keeps
relearning: a median of three inside one window is not a defence against a
window that is itself unusual. The generating row is where the last
...
0.227, 0.228, 0.229, 0.233, 0.233, 0.234, 0.236, 0.237 and 0.240 s, and at sixty-four tokens rather than twelve 1.266, 1.266,
...
110-token prompt has gone from 1.951 s to 0.103 s. Six changes took the
...
costs 1.202 s that way against 0.373 s, which is the quantized activations
...
| workers | wall | processor | cores busy | speedup |
|---|---:|---:|---:|---:|
| 1 | 25.309 s | 25.90 s | 1.02 | 1.00x |
| 2 | 8.703 s | 26.56 s | 3.05 | 2.91x |
| 4 | 6.245 s | 31.56 s | 5.05 | 4.05x |
| 7 | **5.221 s** | 40.16 s | 7.66 | **4.85x** |

**The fourth column is why the third cannot be read as a parallel speedup.**
A worker count is not a core count here: the submitting task takes a share
as well, so one worker is one core and two workers is three, and the step
from the first row to the second is a step from one core to three. Amdahl
fitted across it returns a negative serial fraction, which is what fitting a
curve to two different quantities does. Fitted from the two-worker point
instead, on cores rather than workers, it gives between two and nine per
cent depending on which pair is taken, and a curve that will not settle on
one number is not a curve to quote a number from.

What the table does say plainly is that seven workers reach 4.85 times what
one does, on 7.66 cores of a machine that has eight.

**The share that does not parallelize is smaller than this section used to
say.** It was read off a fifteen per cent serial fraction that the table
above no longer supports, and the honest version of the claim is narrower:
seven workers on 7.66 cores return 4.85 times one worker on one core, so
between a fifth and a third of what the extra cores could give is not
arriving, depending on which pair of rows the arithmetic is done between.
Where that goes is not settled by this table, and the sections below that
tried to halve the strip kernel are the record of looking for it.
...
**The device's short prompt read 1571.4 in three sittings running, 1182.8
in one, and 1549.3 here**, with llama-bench on the same row reading 1647.3
against 1560.0, 1543.3 and 1645.5. Neither side is a change to the code, and the
paragraph under the 1419-token table has the alternated readings that say
so. What this row has spent ten sittings saying is that it is inside the
spread, and the one reading that fell outside it turned out to be the
...
reasons that are not the program: llama.cpp's own processor prompt has read
397.6, 392.8, 396.1 and 387.5 at 110 tokens across four of them, and 319.3,
312.4, 305.5 and 307.2 at 1419. That is what taking both sides in one sitting
is for, and it is why the row worth reading is the one that moved against the
machine rather than with it -- the generated token, which has gone 1.07,
1.04, 1.03, 1.02 while everything around it wandered.
...
1486.5, 1195.7, 1235.9 and now 1208.8 on it, because a tenth of a second
of work never brings the part above about 1.5 GHz -- this sitting's line
says 1104 of 2700 -- and where inside that low band the clock sits decides
the cell. The processor's short row moves on the other side as well:
`llama-bench` read 294.0 two sittings ago, 370.7 in the next and 294.3 in
this one, on the same binary and the same file. **A hundred and ten tokens is too short a measurement for
...
them, said in this paragraph that no device code had changed and that the
part's clock was the reason, and left them standing as measured. Three hours
later the same two binaries -- alternated, four readings each -- read 0.812
to 0.841 s on the 1419-token device prompt where that sitting read 1.162 to
1.186, with the part at 1889 to 1944 MHz rather than 1755. The rows above
are the second reading and the ratios are back where eight sittings have
had them. Both readings are kept here because the pair is the finding: the
same binary, the same host, the same command, **1.41 times apart on the
strength of a clock state**, and only the second sitting looks like the
history. A median of three inside one window is not a defence against a
window that is itself unusual, which is the second time this section has had
to say that in one day.

**The processor's long prompt is ahead of llama.cpp -- 275.6 tokens a
second against 254.5 at 1419 -- and its short one is behind, 361.8 against
378.9 at 110.** The short row
...
pp110 has read 378.9 ± 1.4, 369.2 ± 7.9, 367.9 ± 6.6, 375.3 ± 3.8, 369.3 ± 11.1, 340.4 ± 24.4, 371.3 ± 16.6,
346.8 ± 34.5, 339.4 ± 27.2, 341.3 ± 25.0 and 342.4 ± 25.6 across nine
sittings, and its device pp110 1643.9, 1539.3, 1647.3, 1560.0, 1543.3, 1645.5, 1538.3, 1579.1,
1640.8, 1583.3 and 1654.0, against this program's 1527.8, 1486.5, 1549.3, 1506.8, 1182.8, 1571.4,
1571.4, 1571.4, 1549.3, 1617.6 and 1486.5. The claim those rows
...
**And the generating row is 1.08 behind on the processor and 1.07 on the
device**, after `### The share count a generated token was tuned to` found
that constant had been chosen against the pool the same change replaced.

Both tables are one sitting, taken with `llama-bench` on the same
...
figure here, 5.148 s, sits beside the 5.088, 5.130, 5.123, 5.034 and 4.972
five other windows read since, so it is a sitting and not a window.
...
The processor is **ahead at the longer prompt** -- 1.08 at 1419 tokens --
and 1.05 behind at 110; the device is 1.04 behind at 1419 and 1.08 behind at
...
three runs it is **±3.6 on 1760.5 at 1419 tokens against ±20.1 on 1643.9 at
110, both on the device** -- a fifth of a per cent against one -- and the same 1419 figure read 224.9 ±20 half an hour earlier on a
...
On the processor at 110 tokens: **1.1 times slower generating and 1.1 times
slower reading a prompt** -- the generating figure has read 1.1 four times, 1.2 eight times, 1.3 and 1.4 across sittings, the first four of them
...
reading of this table said 3.3 and 16. On the device, **1.1** and **1.0**,
where the sittings before this one said 1.1 and 1.3, then 1.1 and 1.5, then 1.1 and 1.4, then 1.1 and 1.5, then 1.1 and 1.6, then 1.1 and 1.5, then 1.1 and 1.5, then 1.1 and 1.5, then 1.1 and 1.6, then 1.1 and 1.5, then 1.2 and 1.4, then 1.2 and 1.7, then 1.2 and 1.6, then 1.2 and 1.6, then 1.2 and 1.7, then 1.3 and 1.7, then 1.2 and 2.1, then 1.3 and 2.1, then 1.3 and 2.3, then 1.2 and 2.4, then 1.3 and 2.5, then 1.3 and 2.7, then 1.4 and 2.7, then 1.4 and 2.6, then 1.4 and 2.7, then 1.4 and 2.9, then 1.4 and 2.3, then 1.4 and 2.1, then 1.4 and 2.5, then 1.4 and 2.5, then 1.4 and 2.2, then 1.4 and 2.5, then 1.4 and 2.3, then 1.4 and 2.5, then 1.4 and 2.4, then
...
The device generating row was the noisiest here for a long time: 50.3 t/s
now, against 50.6, 50.8, 51.4, 50.2, 50.5, 50.6, 50.3, 49.5, 52.6, 52.5, 50.6, 54.2, 50.3, 50.3, 50.3, 50.4, 50.0, 49.9, 50.6, 50.6, 46.6, 47.8, 47.0, 48.5, 48.0, 47.7, 48.3, 48.2, 48.4, 47.3, 48.0, 46.2, 44.4, 48.5, 44.5, 44.4, 45.8, 43.8, 42.3, 40.1, 40.3, 39.8, 39.4, 40.6, 40.6, 40.5, 40.4, 40.2, 40.1, 40.7, 38.9, 40.9, 41.0, 40.7, 41.6, 41.3, 40.6, 41.0, 41.5, 41.2, 40.8, 28.1, 30.9, 27.1, 31.0, 30.9, 27.3, 26.9, 31.0, 31.2, 28.1,
...
has thresholds in different places. The first version of this table had ours
stop scaling between eight and sixteen -- 250.6 to 266.7 at a short context,
175.3 to 182.0 at a long one -- where llama.cpp's roughly doubles, and that
step was a constant in a shader rather than anything about the arrangement:
`### The pass over the weights a tenth row cost` below is what it was and
what it is now worth. What remains after it is a curve that still climbs
more slowly than theirs across that step, and one thing worth naming:
...
- **llama.cpp's own step from eight to sixteen is 2.37×**, which is more than
  ...
  1419 positions). Something switches on their side too. Ours now switches
  as well -- the matrix instruction takes a round from nine sequences up,
  which is what the two sections below are about -- and the step from eight
  to sixteen is 1.94 where it was 1.06.
- **It is not the matrix attention kernel**, which is what this section
  guessed first. Sixteen rows is where a batch switches to it and a round is
  excluded from it, so the guess fitted -- and the phase clock says attention
  is 0.0 per cent of a round's accounted time, because the whole layer goes
  down as one submission. The guess was wrong and the profile was what said
  so.
...
### The pass over the weights a tenth row cost

`row_product.comp` reads a weight once and multiplies it into every vector of
the batch it holds. How many that is, is a constant in the shader -- `GROUP`
-- and it was eight, so a batch of nine went down as **two dispatches, and
the second read every weight in the model again**. The cost of that is a step
and not a slope:

| sequences in a round | before |
| ---: | ---: |
| 8 | 2.005 s |
| 9 | **3.557 s** |
| 10 | 3.614 s |
...
Seventy-eight per cent more time for twelve per cent more rows, and then
almost nothing across the next seven. Nine to sixteen all cost the same
thing, which is two passes over a gigabyte of weights, and the shape of that
plateau is what named the constant.

The source is already compiled twice -- once as it stands and once with
`SINGLE`, which sets `GROUP` to one for a generated token, because a batch of
one on the eight-wide kernel carries eight accumulators and eight kilobytes
of shared memory for a reduction seven eighths of which is thrown away. So
the fix is a third compilation, with `GROUP` at sixteen, bound when the batch
is between nine and thirty-one. Below nine the eight-wide kernel is still the
right one; at thirty-two the matrix product takes over, and a tile there is a
hundred and twenty-eight vectors wide, so sixteen rows through it would be
seven eighths invented zeros.

Alternated, three rounds each, TinyLlama-1.1B Q8_0 on the device, load 0.56
before the first:

| | before | after | |
| --- | ---: | ---: | ---: |
| 9 sequences, 7-token prompts, 64 generated | 3.611 s | **2.827 s** | 1.28 |
| 16 sequences, same | 3.861 s | **3.077 s** | 1.26 |
| 9 sequences, 1419-token prompts, 32 generated | 2.319 s | **1.944 s** | 1.19 |
| 16 sequences, same | 2.870 s | **2.505 s** | 1.15 |

Every one of the twelve readings on each side is on the same side of the
other's, which is not usually true of a five per cent noise floor.

**The step at nine is smaller and it is still there** -- 2.002 s at eight
against 2.810 at nine -- and that is the honest part of this. One pass of
the sixteen-wide kernel is not one pass of the eight-wide kernel: it does
sixteen multiply-adds per weight where the other does eight, and it holds
sixteen kilobytes of shared memory where the other holds eight, which halves
how many workgroups a compute unit can keep in flight. Reading the weights
once instead of twice buys more than that costs -- 1.15 to 1.28 -- but it
does not buy the whole difference. A batch of nine still pays for the seven
vectors it does not have.

**Nothing outside a round moved, and that was checked rather than argued.**
A generated token is one vector and goes to the narrow kernel; a prompt is
read a hundred and twenty-eight tokens at a time and goes to the matrix one.
The two binaries alternated on the 1419-token device prompt read 1.162, 1.176
and 1.164 s against 1.173, 1.176 and 1.186, and on the 110-token one 0.123
and 0.123 against 0.123 and 0.122. Both runs' digests are what they were.

### Where a round of seventeen belongs

The change above left a band behind it. Between `Wide_Group` and
`Tile_Least` a batch was served by neither kernel well -- too many vectors
for one dispatch of the sixteen-wide row product, too few to be let into the
matrix product -- and a round there cost more than a round of twice the size:

| sequences in a round | before | after |
| ---: | ---: | ---: |
| 16 | 3.153 s | 3.112 s |
| 17 | 5.431 s | **4.155 s** |
| 24 | 5.704 s | **4.336 s** |
...
**Seventeen sequences cost more than thirty-two.** `Tile_Least` was
thirty-two, so seventeen went down as two dispatches of the row product --
two passes over every weight in the model -- while thirty-two went to the
matrix product, whose tile is a hundred and twenty-eight vectors wide
whether it is handed seventeen or thirty-two. The matrix product costs the
same thing across that whole span. So the boundary belongs immediately above
the row product's group and not at a round number, and it is now written
`Wide_Group + 1` so the two cannot drift apart.

With a 1419-token context the same three rows read 2.849, 2.986 and 3.094 s
against **2.140, 2.183 and 2.245** -- 1.33 to 1.38. Sixteen is untouched,
which is the point of the boundary being where it is.

### Three ways of reading a batch that were measured and not kept

The kernel above spends most of its time on one loop: for every weight it
decodes, it multiplies that weight into sixteen accumulators and reads
sixteen activations to do it. Those sixteen live one vector-length apart --
eight kilobytes, for this model -- so the loop touches sixteen cache lines to
use sixteen floats, and touches them again for the next column.

**How much that spread costs was measured before anything was built.** Two
builds that compute the wrong answer, alternated against the right one on a
round of sixteen:

| | reads | 64 rounds |
| --- | --- | ---: |
| as it stands | sixteen loads, sixteen lines apart | 3.08 s |
| the same loads inside one line | `x[from[0] + col + k]` | 2.16 to 2.45 s |
| one load, sixteen multiply-adds | `x[from[0] + col]` | 1.78 s |

So the spread is worth about **1.34** and the loads themselves about as much
again. That is a real ceiling and worth chasing. Three things were built to
chase it and all three are slower than the loop they replace:

**A transposed batch** -- the sixteen values a weight needs made contiguous,
which is what the middle row above measures -- reads **4.02 s**, a third
worse. Contiguity across `k` costs contiguity across lanes: eight lanes of a
row read eight consecutive columns today, one thirty-two byte fetch, and
under a transpose they read eight blocks sixteen floats apart. The layout
cannot have both and the one it has is the better one.

**Four columns to a load**, `vec4` instead of four scalars, reads the same
3.10 s. The compiler did not merge them, and it cannot: the base is
`vector * columns` and nothing tells it `columns` is a multiple of four.

**A window of the batch staged in shared memory** -- two hundred and fifty-six
columns at a time in the array the reduction uses later -- reads **12.9 s**
as first written and **5.3 s** once the bank conflicts were out of it. Eight
lanes reading a block apart land on one bank of thirty-two, which is eight
times the reads; giving each lane one word of every block instead of every
word of one block fixes that and makes the weight read contiguous as well.
It is still 1.7 times slower than doing nothing. The cache is already serving
these reads out of the nearest level, and staging them by hand costs the
barriers and the shared reads without removing anything.

**And a fourth, which is not about the reads at all.** The kernel does
sixteen multiply-adds per weight whether the batch holds sixteen vectors or
nine, because `GROUP` is a constant and the loop is unrolled around it.
Bounding the loop by what the dispatch actually holds -- one word, uniform
across the workgroup -- reads **5.72 s against 2.85 at nine sequences**, two
and a half times worse: a dynamically bounded loop is not unrolled, and an
array indexed by a variable in a loop that is not unrolled leaves registers
for scratch memory. The shader's own note says the trip count has to be
constant to unroll. It is right, and this is what it costs to disbelieve it.

### A tile the size of the batch

The section before last put a round of seventeen sequences on the matrix
product because the matrix product was the cheaper of two bad options there.
It was cheaper for a reason that is also the reason it was bad: **a tile costs
what its width costs whether the batch fills it or not**, so seventeen
vectors through a hundred-and-twenty-eight-wide tile pay for a hundred and
eleven vectors of zeros, and so do thirty-two.

That was measured before it was fixed. With `Tile_Least` dropped to nine as a
probe, the wide tile reads the same thing at nine rows as at sixteen -- 3.98
and 4.14 s -- and a weight pass on its own, which is what a batch of one
costs, is 1.28. Most of the rest is arithmetic on invented zeros.

`matrix_product.comp` is now compiled twice more, with `NARROW`, which sets
`TILE_V` to thirty-two instead of a hundred and twenty-eight. The weight tile
is the same either way; it is the vector side that shrinks, from eight
accumulator fragments to two. Alternated, three rounds each, load 2.25 before
the first -- which is above this file's usual gate and is why every reading is
given rather than a median:

| sequences | before | after | |
| ---: | ---: | ---: | ---: |
| 9 | 2.885, 2.865, 2.950 | **2.046, 2.003, 2.080** | 1.41 |
| 16 | 3.081, 3.116, 3.215 | **2.132, 2.175, 2.104** | 1.46 |
| 32 | 4.480, 4.551, 4.543 | **2.280, 2.298, 2.281** | 1.99 |
| 64 | 5.248, 5.169, 5.109 | **4.261, 4.193, 4.266** | 1.21 |

and with a 1419-token context, two rounds each, 1.29, 1.25, 1.80 and 1.19.
**A round of sixteen on the device now generates 491.6 tokens a second where
it generated 266.7 this morning**, and one of thirty-two generates 897.9.

**Both boundaries were found rather than chosen.** `Tile_Least` is
`Batch_Group + 1`: from two to eight the row product and the narrow tile
measure the same to within noise -- 1.812 against 1.817 at two, 2.048 against
2.009 at eight -- and the row product is exact where the tile is half
precision, so the exact one keeps the counts it does not lose on. Above the
narrow tile's own width a batch is several narrow tiles, and whether that
beats one wide tile is a second question with a different answer: it wins at
thirty-three, forty-eight and sixty-four -- 3.604, 3.932 and 4.267 against
4.340, 4.743 and 5.135 -- and loses at a hundred and twenty-eight, 8.73
against 6.20. So `Narrow_Limit` is sixty-four and is a constant of its own
rather than the tile's width reused.

**This makes the wide row kernel of two sections ago unreachable on this
host.** Nine to thirty-one vectors was the band it was written for and the
tile now takes that band; a build with the wide kernel switched off measures
the same as one with it, 2.09 against 2.37 and then 2.40 against 2.08 in
alternation, which is noise on both sides. It stays because a device without
the matrix instruction still needs it, and that is what it is now for.

### Two things this did not need to do

**Half-precision activations for the wide row kernel** were the next thing
planned, and the section above deleted the case for them: the kernel they
would speed up is no longer reached on a device that has the matrix
instruction. Building it would be a change measurable by no figure this file
publishes, so it is not built. On a device without the instruction it would
still be worth what it is worth, and that is a thing to measure on such a
device rather than to guess here.

**And the processor's round path has no defect of its own.** It was the third
thing on the list, on the strength of being 1.12 behind llama.cpp at sixteen
sequences. What the phase clock and a profile say is that it is behind by the
same amount everywhere and for the same reason:

| | ours | llama.cpp |
| --- | ---: | ---: |
| one sequence | 33.9 t/s | 39.5 |
| sixteen sequences | 218.7 | 242.1 |
| what sixteen amortize to | **6.4×** | 6.1× |

**It amortizes better than llama.cpp does and starts from further back.**
There is no threshold, no band, nothing that a count of sequences reaches and
falls off; the round is 1.11 behind because a token is 1.17 behind. Feeding
is sixty-four per cent of it at every count from one to sixteen and
projecting twenty, which is where the weights are. A profile of the round
puts fifty-three per cent in the strip kernel and twenty-one in the procedure
...
### A word at a time, and the format nobody had measured

**Every figure in this file is Q8_0**, and that turns out to have been hiding
something. Generating sixty-four tokens on the device, one sitting, against
llama.cpp on the same files:

| | file | generating | ours | llama.cpp | |
| --- | ---: | ---: | ---: | ---: | ---: |
| Q4_K_M | 637 MB | 1.884 s | 34.0 t/s | 87.1 | **2.56** |
| Q5_K_M | 746 MB | 1.822 s | 35.1 t/s | 76.6 | **2.18** |
| Q8_0 | 1116 MB | 1.280 s | 50.0 t/s | 55.5 | 1.11 |

**The k-quants move sixty per cent of the bytes and take a third more time**,
and where llama.cpp is twice as fast generating with a four-bit format as
with the eight-bit one -- as it should be, there being half the bytes -- this
program is slower. Against llama.cpp the gap on the format most published
models actually use is **2.2**, where the format every figure here is taken
on is 1.12.

**It also settles a thing this file had assumed the other way.** Generating on
the device is not bandwidth-bound: half the bytes could not take more time if
it were. It is bound by what the kernel does per byte.

Part of that is now fixed. `row_product.comp` reads a quantized weight through
`byte_at`, which is a word load, a shift and a mask -- so a format whose
quants are read one byte at a time pays four loads where one would do. The
Q8_0 branch has read words since it was written, including the shift that
handles a run not starting on a word boundary; the k-quants had not. They do
now, through a `word_of` written from Q8_0's own lines:

| | before | after | |
| --- | ---: | ---: | ---: |
| Q4_K_M, 64 generated | 1.882, 1.904 s | **1.690, 1.690 s** | 1.11 |
| Q5_K_M, 64 generated | 1.821, 1.811 s | 1.836, 1.841 s | -- |

**Bit-exact: every format's digest is what it was**, because the elements are
taken in the same order and each is the same number.

**Q5_K measured flat and the way it got there is the interesting part.** Read
naively -- both its runs of words fetched inside the group loop -- it was
*worse*, 2.486 s against 1.821. Its thirty-two fifth-bit bytes are the same
for all four of a block's groups, two bits of each serving one pair of
sub-blocks, so fetching them per group is three quarters waste -- and the
byte-at-a-time form had been paying that too. Read once for the block they
come out level. A change that is neutral after being wrong twice is worth
recording as neutral.

**And 1.11 of 2.56 was not the answer.** The byte loads were a real cost and
they were not the main one. What was left is `### A lane a block, where the
block is two hundred bytes` below, which took most of the rest.

### A lane a block, where the block is two hundred bytes

The section above got a ninth of the k-quant gap and said the byte loads were
not the main cost. An ablation said where the rest was: a build of Q4_K with
its weight reads removed -- the wrong answer, none of the loads -- generated
in 1.231 s against 1.677, so **the loads were twenty-seven per cent**, which
is a great deal for a format that fits in nine per cent of the file's bytes.

The reason is the arrangement, not the reads. Eight invocations share a row,
and each took **a super-block of its own**. A Q4_K super-block is a hundred
and forty-four bytes, a Q5_K one a hundred and seventy-six, a Q6_K one two
hundred and ten -- so the eight lanes of a row fetched from eight addresses
that far apart, eight cache lines to use thirty-two bytes, and again for the
next word.

They now share the block and take consecutive words out of it. A super-block
divides eight ways along lines the format already has: Q4_K and Q5_K as four
groups of two halves, Q6_K as four unit-runs of two. Alternated, three rounds
each, sixty-four tokens generated on the device:

| | before | after | |
| --- | ---: | ---: | ---: |
| Q4_K_M | 1.884 s | **0.996 s** | 1.89 |
| Q5_K_M | 1.822 s | **1.084 s** | 1.68 |
| Q8_0 | 1.280 s | 1.279 s | -- |

**Q4_K generates 64.3 tokens a second where it generated 34.0**, and the gap
to llama.cpp on it is **1.36 where it was 2.56**. Every digest is what it
was.

**Q6_K is most of that and it was not where I expected it.** Q4_K_M and
Q5_K_M are not one format: their output projection and some of their
attention tensors are Q6_K, and giving that branch the same treatment was
worth a further 1.37 on both files after the other two had been done. A
"_M" file is a mixture and its slowest format decides it.

**It does not work for a thirty-two element block, and the measurement said
so before the reasoning did.** Q8_0's block is thirty-four bytes: sharing it
gives each lane one word, four elements -- and the per-block setup, a
half-precision scale and two offsets, is then done eight times over for four
elements of work. It reads **1.441 s against 1.251**, fourteen per cent
worse. The trade is between the scatter a lane-a-block causes and the setup a
shared block repeats, and two hundred elements a block is on one side of it
while thirty-two is on the other.

**And two formats are left out for a reason worth recording.** Q2_K and Q3_K
take the same treatment and get a second saving with it -- their four groups
are four two-bit fields of the same sixteen bytes, so a lane-a-group read
them four times over. Done, they made **Q8_0 ten per cent slower** while its
own branch stood byte-identical: 1.300, 1.315 and 1.317 s against 1.151,
1.193 and 1.195. A shader is one module and its registers are allocated for
the worst branch in it, so two branches holding four words across a loop cost
every other branch its occupancy -- which is the same thing
`matrix_product.comp` was split in two for, and measured at twenty-one per
cent there. Reverting those two put Q8_0 back to a wash. **Splitting the row
...
### The element-wise work of a layer, on the pool

The section above leaves a generated token 91 per cent inside a product, 6.1
per cent between one and the next, and 2.2 per cent preparing one. This is
about the 6.1.

**What is between two products is element-wise, or it is a reduction, and
that is the whole of what decides where it runs.** A share of a job is a
range, so cutting by element is exact where the answer at an index reads that
index and nothing else -- the gated middle, the residual add -- and wrong
where it does not. A normalization reads the sum of the whole vector before
it writes any of it, so it cannot be cut that way at all, and the batched
path's own shares say the same thing by cutting themselves by position
instead, which is what a batch has more than one of and a token has not.

**So the gated middle goes to the pool and the normalizations stay.** The
gate is the wide half of the feed-forward -- 5,632 elements against the
model's 2,048 -- and each of its elements is an exponential and a multiply
where a blend's is a multiply and an add. `tests benchmark` prices the
activation at 2.13 ns an element against 0.26 for a quantized row product,
which is why it carries a weight of eight when it asks the pool whether it is
worth waking: the bound stays one number for every caller and the caller says
what its elements cost.

**The bound moved again, and it was swept rather than reasoned.** At a
million, 65,536, 32,768, 8,192 and 2,048 elements, TinyLlama Q4_0 in panels
reads **63.9, 64.8, 66.1, 65.9 and 65.8** tokens a second, medians of three,
the load rising through the sweep so that the later points are the
disadvantaged ones. A million shares nothing; 65,536 shares a token's
attention and not its gate; 32,768 shares both; the two below it also admit
the residual joins and buy nothing more. **So the joins are written as
shares and run on the submitting task at every width this repository has a
model of** -- 2,048 elements against a bound of 32,768 -- and what would
change that is a wider model, which is the bound's business rather than the
call site's.

**The rotation and the cache write were left alone, measured.** A profile
puts `Apply_Rotary` at 0.037 ms a token, which is what one wake and one
barrier cost, so there is nothing in it either way.

**Packing the activation on the pool was built and refused.** Before every
product the vector it multiplies is rounded to a byte an element, and that
is 0.33 ms of a token on the submitting task -- the largest single thing left
after the gate. It is already a share when there is enough of it, and the
bound that decides is a count of blocks; dropping that bound from 256 blocks
to 64 and to 32 reads 65.8 and 65.9 tokens a second against 66.3. **A
generated token packs 64 blocks and posting them costs more than doing
them**, which is what the bound was put there to say and now says with a
number.

**And a tagged type declared inside the layer loop costs 2.3 microseconds a
layer to elaborate.** The share that attention runs on was declared where it
is used, which is inside a loop that runs twenty-two times a token. Three
more empty ones in the same block measured **one per cent of the token**, so
one is about a third of that; the share is declared once for the whole call
now and carries what changes per layer -- the cache offsets, the window's
first position, the sink -- as fields. This is the second time in this file
that something with no arithmetic in it has been worth a per cent, and both
times a profile said nothing and a controlled comparison said it.

**One thing that measured nothing is kept anyway.** `RMS_Norm` zeroed its
whole target and then wrote every element of it, which is a pass across the
activation for nothing, forty-four times a token. Removing it reads inside
the spread. It is gone because it is work for nothing, not because it was
worth anything.

**What the round is worth, alternated three rounds against three, 128 tokens
generated:**

| | before | after | |
| --- | ---: | ---: | ---: |
| Q4_0 in panels | 65.12, 65.13, 65.28 t/s | 66.11, 66.13, 66.46 t/s | 1.015 |
| Q4_K_M in panels | 53.46, 54.06, 54.14 | 54.13, 54.67, 54.87 | 1.011 |
| Q8_0 | 38.90, 39.02, 39.19 | 39.35, 39.43, 39.45 | 1.008 |
| the 1419-token prompt, Q8_0 | 4.504, 4.539, 4.592 s | 4.575, 4.566, 4.561 s | a wash |

**And what is left is one loop, in a package that may not reach a pool.**
The largest single item on the submitting task is now the greedy selection:
32,000 iterations reading two boolean arrays and calling one function that is
not inlined, at about thirty cycles a token identifier, which is **0.26 ms of
a 15 ms token**. It is element-wise and would divide perfectly. It is in
`Model_Runner.Sampling`, which decides what a model says and has no business
knowing what a host has; giving it a pool is a seam rather than an
optimization, and it is not in this change.

### The seam, and the last loop through it

The section above ends by naming what was left: the greedy selection, 32,000
token identifiers walked on the task that had just finished waiting for five,
about a quarter of a millisecond of a fifteen-millisecond token. It also
names why it was left -- the walk is in `Model_Runner.Sampling`, which
decides what a model says, and the pool is in `Model_Runner.Backend.CPU`,
which asks the platform what it has and creates tasks. A package that may be
compiled into a program with no backend at all cannot depend on one.

**So neither of them depends on the other, and both depend on two
interfaces.** `Model_Runner.Shares` declares `Work` -- something a range of
items can be asked of -- and `Team` -- something that can run the pieces of a
job at the same time. The pool wears `Team`; anything that wants sharing
wears `Work`; `Task_Item`, which the engine's own shares have used since
attention was shared out, is now a name for `Shares.Work` rather than a
second interface saying the same thing. Sampling's context clause names
`Model_Runner.Shares` and nothing else new, and the pool becomes a team
through one function on it.

**What is hard here is that both walks are reductions.** Everything shared
out before this was element-wise: the answer at an index reads that index,
so a share computes what the whole computed and where the cuts fall does not
matter. A walk looking for the highest logit is not that. Cut it carelessly
and the answer depends on how many workers a machine happens to have, which
is the one thing this program will not do -- `--threads` may change what a
run costs and may not change what it says.

**It is cut anyway, and what makes it sound is that no block reduces into a
shared place.** The vocabulary is divided into a fixed fifty-six blocks --
fixed, so the blocks do not move when the team does -- each block answers
into a slot of its own, and the slots are combined afterwards in block
order, taking a later one only when it is strictly higher. That reproduces
the tie-breaking of a single walk exactly: the lowest token identifier wins.
The same shape carries the other walk, where the slot holds the first logit
of its block that is not a number and the combination reports the first
block that found one.

**Whether it is worth waking anyone for is the pool's own bound, and the
walk says what its elements cost.** A generated token's greedy walk reads two
mask arrays and searches the window the repetition penalty keeps -- about six
unpredictable branches an identifier, against a multiply and an add for the
blend the bound was set on -- so it carries a weight of eight, exactly as the
gated middle does. **The walk looking for logits that are not numbers carries
no weight and stays below the bound**, which is right: it is a bit test an
element and there is nothing in it to divide.

**One mistake, and the test found it rather than a measurement.** `Sharing`
opened the pool before handing it out, which seemed tidy -- a team that works
the moment you have it. Opening rendezvouses with the workers, and a caller
that asks for a team in the same declarative part that declares the pool is
waiting for tasks the language has not activated yet. The suite hung. Opening
is left where it was, in the first job posted; a team is the reference and
nothing more.

**Held by equality at every worker count.** The test samples the same logits
with no team and with teams of one through eight workers and asserts the same
token, and it plants the case a wrong combination would get wrong: the
highest value twice, in blocks a long way apart, so that a combination in the
wrong order takes the higher identifier. It asserts the tie went to the lower
one. Then it does the same for a logit that is not a number, planted twice,
and asserts both paths name the first.

**What it is worth, alternated four rounds, both orders.** The first three ran
the old arm first and the fourth ran it second, because the first three showed
the 1419-token prompt slower in the new arm every time and a difference that
follows the order rather than the code is the machine warming between them --
which reversing confirmed, the prompt going the other way at once. **A fifth
round was discarded and is recorded here**: its two arms read 5.677 and 4.599
seconds on that prompt, a fifth apart, which is not a measurement of anything
this repository changed.

| | before | after | |
| --- | ---: | ---: | ---: |
| Q4_0 in panels | 63.99, 64.52, 64.41, 63.10 t/s | 65.38, 65.08, 65.04, 66.03 t/s | 1.016 |
| Q4_K_M in panels | 52.92, 52.79, 53.31, 53.12 | 53.91, 53.90, 54.51, 55.19 | 1.022 |
| Q8_0 | 38.44, 38.54, 38.86, 38.59 | 38.75, 38.85, 39.09, 39.11 | 1.010 |
| the 1419-token prompt, Q8_0 | 5.298, 5.338, 5.310, 5.398 s | 5.366, 5.341, 5.432, 5.383 s | a wash |

Every reading of the four-bit and the k-quant arms is outside every reading of
the other; the eight-bit rows overlap by one, which is what a two-thirds of a
per cent difference looks like on a part that moves this much between
sittings.


### A cache that holds only the window

Three architectures here slide a window: gemma2 on every other layer, gemma3
on five layers in six, gpt-oss on every other one. Until now that narrowed
what a position could **read** and nothing else -- every layer was given the
whole context, including the twenty-two of gemma3's twenty-six that can never
look further back than 512 positions. llama.cpp keeps two caches for exactly
this, a full-size one for the layers that attend to everything and a
window-sized one for the rest, and this is that.

**What it costs, on the models this repository has:**

| | the whole context a layer | the window and a batch |
| --- | ---: | ---: |
| gemma3, 26 layers, 22 of them windowed at 512, context 32,768 | 1.834 GB | **0.333 GB** |
| gemma2, 26 layers, 13 of them windowed at 4096, context 8192 | 1.834 GB | **1.434 GB** |

**5.5 times on gemma3, whose weights are 800 MB**, and 1.28 on gemma2 --
which is the honest shape of it rather than a disappointment. A layer holds
its window and a batch, so what it saves is the context less that; gemma3's
window is a sixty-fourth of its context and gemma2's is half of it.

**A ring was the obvious design and is not the one here.** Storing position
P at `P mod cells` keeps the cache full with no copying, and it makes the
positions of a window non-contiguous in memory -- which every hot loop in
attention assumes they are. `Head_Scores_Across` is handed a first key and a
stride and walks; the blend walks the values the same way. Wrapping would
have put a modulus inside both.

**So the window slides instead, and rarely.** A layer holds the window and a
batch, a position sits at its distance from the lowest one the layer still
holds, and when a run passes the end what the window still needs is moved
down to the front. That costs a window of rows moved every batch of
positions -- eight rows for every row written on gemma2, one on gemma3 --
against the six hundred megabytes of weights that position reads. Between
slides the positions are contiguous, every kernel is the kernel it was, and
the arithmetic is in the same order: **both models produce the same text they
produced before, byte for byte, on a 1328-token prompt that slides.**

**One thing had to change shape and it is the seam that made this safe.** The
blend used to be given positions; it is given cells now -- where the rows
sit rather than what they are. Every distance it takes is a difference
between two of them, so subtracting the same origin from both leaves the
arithmetic where it was, and the causal mask, the fall-off with distance and
the softmax are none the wiser. That one substitution is what let a change to
the cache's shape leave three attention kernels, both evaluation paths and
the device untouched.

**The device slides too, and no shader changed.** Its block is this cache's
own length, so a narrower cache is a narrower block; what a slide has to do
beyond moving the host's rows is write the moved ones over, which costs one
`Put_Cache` a layer once every batch of positions. The shaders needed nothing
because everything they do with a position is a difference -- the causal
mask compares a step against a last, the window is derived as `last + 1 -
window`, the fall-off with distance is `abs (step - asking)` -- and
subtracting the same origin from both sides of a difference leaves it where
it was. That is the same property that let the host's blend take cells, and
it is why a change to the cache's shape reached the device without reaching
the device's code.

**The slide settles what the device owes first.** The host's copy of the
cache is deliberately behind -- `Settle_Cache` runs when something is about
to read it rather than at the end of every call, which is the whole of the
difference between reading a 1419-token prompt on the device in 1.000
seconds and in 0.705. A slide is one of those somethings, so it settles; and
it asks only when a layer is actually about to slide, so the lazy settle
still holds for every call that does not.

**A round's rows are different sessions and each holds its window somewhere
of its own**, so the table the round shader reads -- a row's last position
and the block it reads -- now carries a row for every layer as well as every
row, and each layer is handed the offset of its own slice. The whole table
goes over once, so this costs a longer write rather than a write a layer,
and the guard that caps a round is now on rows times layers.

**One thing here is held by construction rather than by a run, and it is
worth naming.** A round that has *slid* would need five hundred and fifteen
positions in each of its members, which on the models to hand is 4608 on
gemma2; the round tests step four. So the per-row, per-layer origin in that
table is exercised with every origin at zero -- a device round on gemma2
gives the mark it gave before, and replacing each row's origin with the first
...
holding position zero, so the format carries an origin and a room for every
layer and the version is two. A file written by the version before it is
refused by version, as it always was, and a session whose geometry differs --
a different window, a different backend -- is refused by name rather than
filled with positions it cannot place.

**Held by the independent implementation, at a run long enough to slide.**
The existing test that a window narrows attention runs eight positions and
never reaches the end of a layer's room; this one runs 518, which is past the
window and a batch, and its last position reads across the rows that moved.
Keeping one row too few fails it. So does moving nothing -- but only at 518:
at 560 it passes, because a window of three reaches three positions back and
forty positions later there is nothing of the move left to be wrong about.
**A test that ran longer would have held less**, which is the opposite of
what one would guess and is why the length is written down beside the reason.
...
specialization constant. That second half is what made this one nearly free:
llama.cpp compiles its mat-vec **once for every column count from one to
eight**, and with the width a constant the pipeline is told, so can this --
a `vkCreateComputePipeline` each and not one word more of SPIR-V.

**What it was costing is visible as a step, and as a curve that goes
backwards.** Four widths, per round of a 1419-token prompt:

| members | a round | tokens a second |
| --- | ---: | ---: |
| four | 25.3 ms | 158.2 |
| **five** | **33.3 ms** | **150.1** |
| six | 34.2 ms | 175.5 |

A column is worth about 1.3 ms inside a width, and the fifth costs eight --
because five sequences ran on the eight-wide kernel and paid for eight.
**Five members came out slower than four**, which is the kind of thing a
curve says out loud.

**A pipeline for each count, alternated three rounds against the four
widths**, every mark unchanged at one through eight and at sixteen:

| members | four widths | one for each | |
| --- | --- | --- | --- |
| two | 0.720, 0.736, 0.738 s | 0.700, 0.702, 0.719 s | 3 % |
| five | 1.061, 1.069, 1.070 s | **0.868, 0.871, 0.871 s** | 18 % |
| six | 1.098, 1.105, 1.107 s | **0.994, 0.999, 1.001 s** | 9.5 % |
| seven | 1.161, 1.169, 1.180 s | **1.091, 1.094, 1.099 s** | 6.5 % |

Better in twelve of twelve. And the curve is a curve again:

| members | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 16 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| t/s | 46.4 | 93.0 | 130.8 | 157.6 | 180.4 | 193.0 | 204.9 | 213.5 | 355.1 |

Five members is 180.4 where it was 150.1, and nothing between one and
sixteen falls below the count before it any more.
...
kept as a mistake, because the objection to it is one sentence long and it
should have been asked before any of it was written down.

The claim was that a `dcn31_program_compbuf_size` timeout in the display
controller flips the device prompt between 0.75 s and 1.08 -- pairs of the
event preceding the slow window, singles leaving it fast, six for six.

**llama.cpp is not affected by the window.** It read 1815 tokens a second in
the slow sitting and 1763 in the fast one, measured alternated against this
program in the same minutes. A display controller shares one memory
controller with everything on the die: whatever it does to this program it
does to llama.cpp as well. **A cause on shared hardware cannot be selective,
and this one is selective, so it is not on shared hardware.** That is the
whole of it, and it disposes of the clock, the power budget, the thermal
state and the memory bandwidth in the same breath -- every one of which was
excluded by measurement earlier, and could have been excluded by this
argument on the first day.

The correlation was also worth less than it looked. Twenty-four events in
seventy-two hours is one every three hours, so almost any moment has one an
hour or two behind it; and twenty-nine of thirty readings are fast, so
"a single, and it stayed fast" is very nearly a description of the
background rather than of anything. What remained was two pairs before two
slow windows. Two points.

**What survives is the constraint, and it is worth more than the guess.**
Whatever flips the window must be something that differs between two
processes on one machine in the same minutes -- this program's code, its
shaders, its allocations, or driver state that belongs to its own context.
It cannot be the part, the memory, the clock or the screen. Everything
excluded so far was excluded for being shared; the search should have been
narrowed to what is not, and was not.

The one thing that does not fit even that is the persistence: the state
survives this program exiting and starting again, which leaves either
something on disk or something the driver keeps per-device rather than
per-context. The shader cache was cleared in the slow window and it stayed
slow, which rules out the obvious member of the first class.

**One measurement from that sitting survives the retraction**, because it
does not depend on the cause at all. A detector took the 1419-token device
prompt once a minute for half an hour: twenty-nine readings between 0.692
and 0.773 s and one at 1.054, against a dozen consecutive readings at 1.08
that afternoon. **The slow window is a state and not a spread** -- it
arrives, it stays for hours, and it leaves. That is worth knowing whatever
causes it, and it is what makes a single sitting's device ratio worthless.

The display events are still in `dmesg` and still unexplained; they are
simply not this, and no amount of them would be.
...
fresh profile said the hottest instructions in the program were still a
sixteen-iteration scalar loop -- a different one, twenty-seven per cent of
everything a single-threaded generated token executed.

It is the activation's side. Q6_K's insertion wants the activation's sum over
every sixteen elements, and it was formed a byte at a time:
...
for Half in 0 .. Blocks * Halves - 1 loop
   ...
   Half_Total (Half) := N.Real (Total) * Scales (...);
end loop;
```

**Two things are wrong with it and the second is the larger.** Sixteen signed
bytes added one at a time is about sixty-four instructions where twelve will
do. And it depends on nothing but the activation -- yet it sits in a kernel
that is called once for every *row tile*, so a thirty-two thousand row output
projection forms the same hundred and twenty-eight sums a thousand times over.
...
| | before | after | |
| --- | ---: | ---: | ---: |
| Q4_K_M, ten generated | 0.223 s | 0.185 s | **17 %** |
| Q5_K_M, thirty-two generated | 0.810 s | 0.688 s | **15 %** |
| Q4_K_M, 110-token prompt | 0.441 s | 0.4435 s | a wash |
| Q8_0, 64 generated | 1.720 s | 1.720 s | a wash, the control |

Five alternated rounds, all five on the same side for both k-quants and the
...
|---|---:|---:|---:|---:|
| three | **1.714 s** | 7.02 s | 0.531 s | 2.07 s |
| four | 1.731 s | 8.72 s | 0.415 s | 2.03 s |
| five | 1.741 s | 8.80 s | 0.438 s | 2.51 s |
| eight | 1.739 s | 8.86 s | **0.351 s** | 3.04 s |

**A generated token is done adding workers at three shares.** From three to
eight the wall does not move -- one and a half per cent, the wrong way --
while the processor time rises by a quarter: the memory path is saturated by
two or three cores and the rest are paying for a queue. That is the bus, measured rather than inferred -- but
...
| | ms a token | share |
|---|---:|---:|
| feed forward, gate up down | 6.163 | 54.2 % |
| attention projections, q k v o | 3.023 | 26.6 % |
| the vocabulary projection | 1.509 | 13.3 % |
| activation and gate | 0.383 | 3.4 % |
| normalization, twice a layer | 0.162 | 1.4 % |
| rotation | 0.106 | 0.9 % |
| softmax over the vocabulary | 0.020 | 0.2 % |
| normalization, once at the end | 0.004 | 0.0 % |
| **what a token costs of these** | **11.371** | |

The same token at `--arith f32` costs 78.650 ms of these, so the products are
**6.9 times** what they were and everything else is where it was. Reading a
prompt, thirty-two a pass, the whole is 3.860 ms a token in the same shape --
5.453 a sitting ago and 8.391 two kernels before that.
...
**The products are 94.1 per cent of what this table measures**, so nothing
...
**And a generated token really costs about 41 ms on one core, against the
11.4 ms this table accounts for.** Both halves of that are measured the same
way -- the table is pinned to one core, and so is the 2.629 s that
sixty-four tokens take at one worker. The missing three quarters is
attention over the cache, which is not measured here because the three blend
kernels are inside `Model_Runner.Llama` with no entry point this tool can
reach. At the first reading of this table that gap was about a ninth; the
products have got several times faster since and attention has not, so it is
now the largest single thing left on the processor -- and it is the one part of a forward pass that
...
Asked again after `### Five jobs a layer where there were two`, which changes
how many jobs a token posts and so could have changed what a job wants:
...
### Several sequences in one pass, which is a round

The section below prices a second caller. This is that, built: `--round N`
opens N sessions on one model, gives each the prompt, and then generates by
rounds -- one token from each member in one pass over the weights. Sixty-four
rounds from a seven-token prompt:
...
Priced at 1.56× for two and 3.23× for four from the batch-size sweep; it pays
**1.68 and 3.36**. A round is slightly the better of the two because a
prompt's rows attend a growing history where a round's attend their own, so a
round's attention is the same work a row every time.

**And the kink is in the thing itself.** Six members read 107.3 tokens a
second where four read 114.7 -- *six members are slower than four*. That is
the strip kernel taking vectors in fours, predicted from the batch-size sweep
before the round existed and now measured in the round. A scheduler gathers
in fours because of this table.

For scale, llama.cpp generates one sequence at 39.3 tokens a second on this
host. Four members is two and nine tenths of that and eight is four and seven
tenths -- which is not a comparison of like with like, and is the point:
nobody serving several callers should be doing it one at a time.

**What it cost to build.** The primitive is `Evaluate_Batch` with two answers
made per row instead of once -- which cache a row attends and where in it the
row sits. The rest of that procedure is untouched: the products were already
over the whole batch, and the normalizations, the rotation, the gated middle
and the joins were already a row at a time. One addition beyond the design:
the final projection over the vocabulary is done for all rows at once for a
round, because a row at a time there would give back a fifteenth of what the
layers just saved.

**The gate** is that every member produces, bit for bit, the logits it would
...
**And on a device it runs its products and its attention.** The fused
half-layer -- the one that normalizes, projects, rotates, writes the cache
and attends in a single submission -- names one cache and one run of
positions for the whole batch, so a round taking it would have every row
attend the first member's history. A round takes the unfused path there
instead: every product goes to the device as a sequence of steps, and so does
attention, reading each row's own block of the device's cache. That last part
is stage three and is below; stage two ran the products there and left
attention on the host.

| members | processor | | device | |
| ---: | ---: | ---: | ---: | ---: |
| 1 | 29.7 ms | 33.7 t/s | 19.9 ms | 50.3 t/s |
| 2 | 18.6 ms | 53.7 | 17.7 ms | 56.6 |
| 3 | 13.9 ms | 72.0 | 12.4 ms | 80.5 |
| 4 | 8.8 ms | 113.3 | 10.0 ms | 100.1 |
| 6 | 9.7 ms | 103.4 | 7.7 ms | 130.4 |
| 8 | 5.7 ms | 175.6 | 6.6 ms | 151.6 |

Read that table knowing what it does not say. At seventy-one positions the
processor is ahead at four members and at eight, and the device is ahead at
one, two, three and six -- an ordering that has changed sign between sittings
on both sides and is not a claim either way. The device column read 5.0 ms at
eight members in the sitting stage two was published from and would not
reproduce below 6.4 in this one, on the binary that produced it as well as on
the one that replaced it. A round this short is sixty-four passes over a
model that fits in cache; what the table is for is the shape of the column,
not the sign of the difference between them. The long-context table further
...
1419-token prompt, on the device, with stage three built:
...
| members | as it was | attention voided | attention's share |
| ---: | ---: | ---: | ---: |
...
processor's remaining gap as *our barrier-per-product structure against
llama.cpp's single parallel region* -- a claim about size as well as shape,
and the size had never been measured. Measured now, by removal, with every
product's share doing nothing at all and everything else left standing:

| | | |
| --- | ---: | ---: |
| sixty-four generated tokens | 1.754 s | **0.112 s** |
| the 1419-token prompt | 4.994 s | **1.268 s** |

Of the 0.112, the phase account says attention, the normalizations, the
rotations and the joins are about 0.09 -- so **the pool's own structure is
about one per cent of a generated token**, and a redesign of it would buy that
and no more. The prompt's 1.268 is the same story at a different scale:
attention is 0.918 of it and the structure is again a rounding error.

So the eight per cent is inside the products, the products are the memory, and
what is left to change about the memory is what the workers ask it for.

**The chunk counter hands out tile numbers in order**, so the first workers to
arrive all start at the front of a job and walk it a tile apart -- one narrow
stream of reads for the memory to serve. The tiles are numbered differently
now: counter value *v* is tile `(v mod Runs) * Per + v / Runs`, with `Runs`
the share count. Worker *w* takes values *w*, *w + Runs*, *w + 2·Runs*, which
are consecutive tiles of stretch *w* -- so each worker walks a contiguous
stretch of its own and the memory sees as many streams as there are workers,
while the counter still balances, because a worker that finishes its stretch
takes whatever is left wherever it is.

Nothing about a value changes: a chunk computes the same rows from the same
weights whenever it is taken, and no chunk is taken twice. Every digest holds.

| | stretches | as it was | |
| --- | ---: | ---: | ---: |
| generating, 64 tokens | **1.7365 s** | 1.747 s | 0.6 % |
| 1419-token prompt | 4.831 s | 4.837 s | level |

Four alternated rounds, every reading of one arm below every reading of the
other. And it shows most where there are most workers to spread: **the
twelve-token run at fifteen threads goes from 0.464 s to 0.401**, its
processor time from 2.62 s to 2.44 -- fourteen per cent of the case this
program has always been worst at, and the reason the fifteen-thread penalty
above now reads seven per cent where it has read as much as thirty-seven.

Two stretches a worker rather than one -- `Runs` twice the share count, so a
...
ranking that led here said five per cent of a generated run is normalizing,
rotating, attending and joining -- a few microseconds of arithmetic each,
posted as a job of the pool. Four of those five were already not posted. The
layer's small work takes
...
and a null pool means the calling task does the whole of it. The comment
above that line says so in as many words, two lines above the call this had
decided to change. It is the fourth time a ranked item's premise has been
sitting in the source, unread.

What was left is **attention**, which is shared by heads and reaches the pool
through `Item.Team` with no such test. A generated token early in a
conversation wakes five workers twenty-two times to divide a cache of seventy
positions between them.

So `Dispatch_Shares` takes a `Cost` -- the arithmetic of the whole job, in
elements -- and the submitting task does the job alone below a floor.
Bit-exact by construction: every item of these jobs is independent of every
other and a share is a range of them, so one task over the whole range
computes what the shares would have, element for element. Single-token
attention says its cost is `Heads * Reserved * Head_Size`: 143 thousand early
in a conversation, 3.0 million after a long prompt.

| | inline | as it was | |
| --- | ---: | ---: | ---: |
| attending | **0.027 s** | 0.053 s | halved |
| generating | **1.748 s** | 1.780 s | 1.8 % |
| 1419-token prompt | 4.811, 4.763 s | 4.814, 4.813 s | level |

**The floor is bracketed rather than reasoned.** Generating sixty-four tokens
after the 1419-token prompt, where the cost is three million and the job must
*not* be taken inline:

| | generating |
| --- | ---: |
| a floor of one million | 1.953 s |
| a floor of eight million | 2.383 s |
| no floor at all | 1.919 s |

Eight million is twenty-four per cent worse, and one million is what no floor
is -- which is the point, since at that context the two run the same code.
The constant has to sit between the context a token wants inline and the one
it does not, and those three readings say it does.

**And a first attempt that was wrong**, kept because the shape of the error
is the useful part. The cost was passed at the five position-shared sites as
well, under the same floor. A prompt is read in batches of a hundred and
twenty-eight, and a batch's normalization is 128 × 2048 = 262 thousand
elements -- under a floor of a million, so it was taken inline, and that is
real work rather than a job too small to share. The long prompt read 5.011 s
against 4.891. Narrowed to the one site that needed it, and the position
sites left alone, because the count test above already covers them.


### Five jobs a layer where there were two

The processor's generating row was 1.11 behind llama.cpp, and a profile says
77.8 per cent of the samples are in the byte row kernel and 10.1 in the
worker's spin -- which says nothing about why. So the question went to the
bus instead:

| | |
| --- | ---: |
| one run alone | 38.6 GB/s of the model |
| two runs at once | **43.1 GB/s** between them |

Twelve per cent the part will deliver and one run was not asking for.
llama.cpp alone reads 42.6, which is the saturated figure, so the whole of
this row's gap is that headroom.

**Two things it is not**, both built and both measured flat, neither kept:

| | 64 tokens |
| --- | ---: |
| one row at a time | 1.808 s |
| two rows at a time | 1.810 s |
| two rows and a prefetch | 1.812 s |

The row kernel walks one row with a multiply-add chain between each load and
the next, so two rows at once are two streams of loads a worker that do not
wait for each other -- nine instructions a row-block against eleven, each row
still summing its own blocks in its own accumulator in the same order, so no
digest moves. It bought nothing, and a software prefetch three hundred and
twenty bytes ahead of each stream bought nothing either. Whatever the twelve
per cent is, it is not memory-level parallelism inside a worker.

**Where it actually is** came from the engine's own phase account, which
`tests speed --budget` has printed all along and which nothing here had read
for this question:

| | | | bytes' share |
| --- | ---: | ---: | ---: |
| feeding | 1.256 s | 69.6 % | 73.6 % |
| projecting | 0.358 s | 19.8 % | 20.0 % |
| reading out | 0.102 s | 5.7 % | 6.3 % |
| attending | 0.049 s | 2.7 % | -- |
| normalizing, rotating, joining | 0.040 s | 2.2 % | -- |

The last column is each phase's share of the model's parameters. The products
are proportional to their bytes, and the four and nine tenths per cent left
over is work that reads almost nothing. Per phase the rate is **41.2 GB/s
feeding, 39.3 projecting, 43.7 reading out**: the one big product of the
vocabulary reaches the saturated figure, the feed-forward nearly does, and
the projections -- the smallest matrices and the most jobs -- do not.

**What is kept** is that a group of matrices reading one input is one job of
the pool rather than one each. A layer's queries, keys and values are one job
and its gate and up another, so a generated token posts **eighty-nine jobs
where it posted a hundred and fifty-five**. The parts share the chunk counter
but not their tiles -- each part's tiles begin at a tile boundary of its own
-- so a part's rows are cut where they would have been cut alone and no
answer moves.

| | grouped | as it was | |
| --- | ---: | ---: | ---: |
| projecting | **0.363 s** | 0.387 s | 6.2 % |
| feeding | 1.267 s | 1.276 s | 0.7 % |
| generating | **1.787 s** | 1.808 s | 1.2 % |

Which is the phase the change aims at moving by six per cent and the run
moving by one, because the phase is a fifth of the run. Every digest is
unchanged.
...
This engine read twelve formats and wrote none. `tests quantize` writes
twelve -- every quantized format `llama-quantize` writes for this model.
Five encode by a closed-form rule; the rest **search**, and that is the
interesting half.
...
| format | how it encodes | same | differing | bytes apart |
| --- | --- | ---: | ---: | ---: |
| q8_0 | a rule | 201 | 0 | 0 |
| q4_0 | a rule | 201 | 0 | 0 |
| q4_1 | a rule | 201 | 0 | 0 |
| q5_0 | a rule | 201 | 0 | 0 |
| q5_1 | a rule | 201 | 0 | 0 |
| q2_K | a search over a scale and a minimum | 201 | 0 | 0 |
| q6_K | a search over a scale | 201 | 0 | 0 |
| iq4_nl | a search over a table of levels | 201 | 0 | 0 |
| iq4_xs | the same, over a super-block | 201 | 0 | 0 |
| q3_K | a search refined level by level | 190 | 11 | **14** |
| q5_K | a search over a scale and a minimum | 193 | 8 | **129** |
| q4_K | the same, a wider window | 194 | 7 | **1,081** |

**Nine of the twelve agree byte for byte, and the three that do not are off
by parts per million** -- 0.03 for q3_K, 0.17 for q5_K, 1.7 for q4_K, of
files between 361 and 902 megabytes.
...
**Four of the searching formats still agree exactly**, which was the open
question when only the rules were written. Q6_K tries nineteen candidate
scales for each run of sixteen; Q2_K fits a scale and a minimum together
over sixteen candidates, judging the miss by its size rather than its
square; and the two IQ formats binary-search a table of sixteen levels
spaced finely near zero. Two implementations comparing floating-point error
sums could have parted company at any near-tie, and in those four they never
did.

**The three that part company are the ones whose search is finest.** Q4_K
sweeps twenty-one candidates and fits both a scale and a minimum at each;
Q5_K does the same over a narrower window with twice the levels; Q3_K takes
one scale and then moves individual levels while the fit improves. Q4_K
parts seven times -- 1,081 bytes of 619,094,016, **1.7 parts per million**,
the first eight bytes of a thirty-six-million-byte `output.weight`. Q5_K
parts eight times by 129 bytes of 756 million, and Q3_K eleven times by
**fourteen bytes** of 473 million. That is a
...
**What is still not written is not quantization.** F32, F16 and BF16 are
conversions rather than quantizations and `--repack` already writes the
first two in memory; MXFP4 is a format this engine reads and
`llama-quantize` does not write, so there would be nothing to check it
against -- and `### What a format costs the predictions` has already said
what MXFP4 costs, which is the question worth asking about it.

### An importance matrix, collected and consumed

A quantizer told nothing about a model spends its levels evenly. One told
which input channels the model actually leans on spends them where they
matter, and llama.cpp's answer is an importance matrix: for every weight
matrix, the sum over a corpus of the square of each activation that met each
of its columns. `tests imatrix` collects one and `tests quantize --imatrix`
uses one.

**It is not the plain search with weights added.** The reference takes a
different path for a weighted quantization -- thirty-seven candidate scales
from -0.9 by 0.05 against twenty-one from -1.0 by 0.1, and the sub-block
scales fitted by a **second** search with five refinement passes rather than
divided by their largest. Two searches where the plain path has one.

**What it buys**, which this repository could not say before. Q4_K `--pure`,
divergence from the eight-bit model over 1,020 positions:

| | divergence, nats | top token agreed |
| --- | ---: | ---: |
| no matrix | 0.061860 | 84.2 % |
| llama.cpp's matrix, held-out text | 0.029365 | 89.5 % |
| llama.cpp's matrix, the measured corpus | 0.027507 | 89.9 % |
| **this collector, held-out text** | **0.027716** | **90.9 %** |
| **this collector, the measured corpus** | **0.026051** | **91.0 %** |

**Twice the accuracy for a corpus and a few minutes.** The held-out rows are
the honest ones: a matrix collected on the very text the divergence is
measured on flatters itself, and the gap between the two rows of each pair --
six or seven per cent, and the same for both collectors -- is exactly how
much. That is why a second 59 KB of prose the corpus does not contain was cut
for this table.

**The seam it needed.** Only the engine knows which matrix a product is about
to read, and a tensor view deliberately carries an address and a length and
no name: a name is a fact about a file and a view is a fact about memory. The
one moment the two are in the same place is `Resolve`, so that is where the
pair is written down, and `Llama.Watcher` is what a session tells. It costs a
null check a product; the twelve-token figure reads 0.340 s either way and
the digest is unmoved.

**Ours is not theirs, and cannot be.** Two matrices collected by two forward
passes hold different sums before any quantizer sees them, so every quantized
tensor differs -- 156 of 156, 3.9 per cent of the file. Byte agreement was
never available here, which is why the check is the divergence and not the
bytes. What the divergence says is that ours does the same job and slightly
better, and the likely reason is visible in the counts: **ours weights 155
matrices where llama.cpp's weights 154.** Theirs excludes the output
projection; ours records it, and `output.weight` is exactly the tensor the
two files disagree about most.

**What it costs**: 410 seconds over 16,000 tokens, against llama-imatrix's
few tens of seconds. The accumulation is serial while the products around it
are parallel, and every product walks a list of two hundred names to turn a

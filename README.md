...
| the matrix product | **0.280 s** | **0.402 s** |
...
**The four-bit k-quant reaches it too**, and it is the format most published
models are shipped in. Only the decode differs -- a value there is the scale
times a nibble less a minimum, computed in binary32 and rounded once into the
tile, where the eight-bit format's is a scale times a byte -- so the two
share a shader and the branch is on a push constant, uniform across a
workgroup and taken once a step. It costs the eight-bit path nothing: 0.287 s
against 0.286 across the same three rounds.

**And it says what to do next.** A `Q4_K_M` file still reads its prompt
slower than the larger `Q8_0` one -- 0.402 s against 0.280 -- because a "_M"
file is a mixture and its six-bit tensors, a sixth of the weights, are still
on the row product. That is the same finding the processor's kernels reached
one format later, and it has the same answer.

**Where it does not run**, which is most places. Thirteen of the fifteen
formats, because `matrix_product.comp` decodes eight-bit blocks and the
four-bit k-quant and nothing else. Every row count the thirty-two-row tile
...
and a batch of forty, which the two tiled formats go through and the other
thirteen take through the row product at a larger shape than they had. The
two paths are held to different bounds because they differ by different
amounts, and both are measured rather than guessed: at most 2.1e-5 between
the row product and the processor, against 7.1e-3 for the eight-bit tile and
7.4e-3 for the four-bit one -- three hundred times as much, and the price of
a half-precision operand.

...
| Sampling | Documented pipeline: vocabulary check, non-finite rejection, masks, per-token biases, repetition penalty, frequency and presence penalties, temperature, top-k, top-p, min-p, renormalize, select. Greedy is tie-broken to the lowest token and consumes no random state; xoshiro256++ seeded per session. `--logit-bias TOKEN=X` adds a fixed amount to a token's logit on both the greedy path and the probabilistic one, which is where the obvious implementation of it does nothing. `--logprobs N` reports what the model made of each position -- the chosen token's log-probability and the N likeliest, from a plain softmax over the raw logits with none of the sampling applied, because a caller asking how sure the model was is asking about the model and not about the configuration. Explaining consumes no random state, so a run with it produces the same text as one without |

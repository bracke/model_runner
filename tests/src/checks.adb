...
package body Checks is
   ...
   begin
      ...
      declare
         ...
         Digest : constant Interfaces.Unsigned_64 :=
           Shader_Generation.Source_Digest
             (Root & "/src/shaders/heads.comp", Found);
      begin
         Result.Performed := Result.Performed + 1;

         if not Found then
            Fail ("src/shaders/heads.comp is missing, and the words "
                  & "compiled from it are committed");
         elsif Digest /= Model_Runner.Shaders.Heads_Digest then
            Fail ("src/shaders/heads.comp has changed since it was "
                  & "compiled; compile it and run 'tests shader' again with "
                  & "every shader named");
         end if;
      end;
      ...
      declare
         ...
         Digest : constant Interfaces.Unsigned_64 :=
           Shader_Generation.Source_Digest
             (Root & "/src/shaders/pack.comp", Found);
      begin
         Result.Performed := Result.Performed + 1;

         if not Found then
            Fail ("src/shaders/pack.comp is missing, and the words "
                  & "compiled from it are committed");
         elsif Digest /= Model_Runner.Shaders.Pack_Digest then
      ...
      declare
         ...
         Digest : constant Interfaces.Unsigned_64 :=
           Shader_Generation.Source_Digest
             (Root & "/src/shaders/unpack.comp", Found);
      begin
         Result.Performed := Result.Performed + 1;

         if not Found then
            Fail ("src/shaders/unpack.comp is missing, and the words "
                  & "compiled from it are committed");
         elsif Digest /= Model_Runner.Shaders.Unpack_Digest then
            Fail ("src/shaders/unpack.comp has changed since it was "
                  & "compiled; compile it and run 'tests shader' again with "
                  & "every shader named");
         end if;
      end;

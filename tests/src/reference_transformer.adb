...
package body Reference_Transformer is
   ...
   --  The grids the four remaining IQ formats index, carried here apart from
   --  the engine's own copies and taken from ggml-common.h as it stands, so a
   --  grid transcribed wrong on one side is not wrong on both. Each entry is
   --  eight magnitudes a byte (IQ3_XXS's four), the lowest byte first;
   --  IQ1_S's are signed bytes, minus one to one.
   IQ2XS_Grid_Ref : constant array (0 .. 511) of Interfaces.Unsigned_64 :=
     [
      16#0808080808080808#, 16#080808080808082B#, 16#0808080808081919#, 16#0808080808082B08#,
   ...
   IQ2S_Grid_Ref : constant array (0 .. 1023) of Interfaces.Unsigned_64 :=
     [
      16#0808080808080808#, 16#080808080808082B#, 16#0808080808081919#, 16#0808080808082B08#,
      16#0808080808082B2B#, 16#0808080808190819#, 16#0808080808191908#, 16#080808080819192B#,
      16#0808080808192B19#, 16#08080808082B0808#, 16#08080808082B082B#, 16#08080808082B1919#,
   ...
   IQ3XXS_Grid_Ref : constant array (0 .. 255) of Interfaces.Unsigned_32 :=
     [
   ...
   IQ1S_Grid_Ref : constant array (0 .. 2047) of Interfaces.Unsigned_64 :=
     [
   ...
   --  A half-precision number from its sixteen bits, as Decode_Half reads one
   --  out of the image: IQ1_M assembles its scale's bits from four words
   --  rather than keeping them together.
   function Half_Of (Raw : Natural) return Long_Float is
      Sign     : constant Long_Float := (if Raw >= 16#8000# then -1.0 else 1.0);
      Exponent : constant Integer := (Raw / 1024) mod 32;
      Mantissa : constant Integer := Raw mod 1024;
   begin
      if Exponent = 0 then
   ...
   --  Byte J of a grid entry, lowest first.
   function Grid_Byte (Entry_Value : Interfaces.Unsigned_64; J : Natural)
     return Natural
   is (Natural ((Entry_Value / (2 ** (8 * J))) mod 256));

   --  One element of an IQ2_XS super-block: seventy-four bytes, the scale,
   --  thirty-two sixteen-bit words -- four a sub-block of thirty-two, one a
   --  lane of eight, its low nine bits a grid index and its top seven a sign
   --  index -- and eight bytes of two four-bit scales, the low one for the
   --  sub-block's first two lanes. A scale s gives a step of d (s + 1/2) / 4.
   function Decode_IQ2_XS
     (Image : Model_Runner.Bytes.Byte_Array;
      Base  : Interfaces.Unsigned_64;
      Index : Natural) return Long_Float
   is
      At_Block : constant Interfaces.Unsigned_64 :=
        Base + Interfaces.Unsigned_64 (Index / 256) * 74;
      Within   : constant Natural := Index mod 256;
      Sub      : constant Natural := Within / 32;
      L        : constant Natural := (Within mod 32) / 8;
      J        : constant Natural := Within mod 8;
      ...
      Word  : constant Natural :=
        Byte_At (At_Block + 2 + Interfaces.Unsigned_64 (2 * (4 * Sub + L)))
        + 256
          * Byte_At (At_Block + 3 + Interfaces.Unsigned_64 (2 * (4 * Sub + L)));
      Scales : constant Natural :=
        Byte_At (At_Block + 66 + Interfaces.Unsigned_64 (Sub));
      Scale  : constant Natural :=
        (if L < 2 then Scales mod 16 else Scales / 16);
      Step   : constant Long_Float :=
        Decode_Half (Image, At_Block) * (0.5 + Long_Float (Scale)) * 0.25;
      Signs  : constant Natural :=
        Natural (KSigns_IQ2XS_Ref (Word / 512));
   begin
      return Step * Long_Float (Grid_Byte (IQ2XS_Grid_Ref (Word mod 512), J))
        * (if (Signs / 2 ** J) mod 2 = 1 then -1.0 else 1.0);
   end Decode_IQ2_XS;

   --  One element of an IQ2_S super-block: eighty-two bytes, the scale, then
   --  thirty-two bytes of grid index -- one a lane -- and thirty-two of whole
   --  sign bytes, one a lane, then eight bytes each holding a sub-block's four
   --  two-bit index tops, the first lane lowest, then the eight scale bytes
   --  as IQ2_XS keeps them. The grid has 1024 entries.
   function Decode_IQ2_S
     (Image : Model_Runner.Bytes.Byte_Array;
      Base  : Interfaces.Unsigned_64;
      Index : Natural) return Long_Float
   is
      At_Block : constant Interfaces.Unsigned_64 :=
        Base + Interfaces.Unsigned_64 (Index / 256) * 82;
      Within   : constant Natural := Index mod 256;
      Sub      : constant Natural := Within / 32;
      L        : constant Natural := (Within mod 32) / 8;
      J        : constant Natural := Within mod 8;
      Lane     : constant Interfaces.Unsigned_64 :=
        Interfaces.Unsigned_64 (4 * Sub + L);

      function Byte_At (Offset : Interfaces.Unsigned_64) return Natural
      is (Natural
            (Image (Image'First + Model_Runner.Bytes.Byte_Count (Offset))));

      Tops   : constant Natural :=
        Byte_At (At_Block + 66 + Interfaces.Unsigned_64 (Sub));
      Grid_Index : constant Natural :=
        Byte_At (At_Block + 2 + Lane) + 256 * ((Tops / 4 ** L) mod 4);
      Signs  : constant Natural := Byte_At (At_Block + 34 + Lane);
      Scales : constant Natural :=
        Byte_At (At_Block + 74 + Interfaces.Unsigned_64 (Sub));
      Scale  : constant Natural :=
        (if L < 2 then Scales mod 16 else Scales / 16);
      Step   : constant Long_Float :=
        Decode_Half (Image, At_Block) * (0.5 + Long_Float (Scale)) * 0.25;
   begin
      return Step * Long_Float (Grid_Byte (IQ2S_Grid_Ref (Grid_Index), J))
        * (if (Signs / 2 ** J) mod 2 = 1 then -1.0 else 1.0);
   end Decode_IQ2_S;

   --  One element of an IQ3_XXS super-block: ninety-eight bytes, the scale,
   --  sixty-four bytes of grid index -- two a lane, each naming four
   --  magnitudes -- then eight thirty-two-bit words, one a sub-block, whose
   --  four seven-bit fields are a lane's sign index and whose top four bits
   --  are its scale, giving a step of d (s + 1/2) / 2.
   function Decode_IQ3_XXS
     (Image : Model_Runner.Bytes.Byte_Array;
      Base  : Interfaces.Unsigned_64;
      Index : Natural) return Long_Float
   is
      At_Block : constant Interfaces.Unsigned_64 :=
        Base + Interfaces.Unsigned_64 (Index / 256) * 98;
      Within   : constant Natural := Index mod 256;
      Sub      : constant Natural := Within / 32;
      L        : constant Natural := (Within mod 32) / 8;
      J        : constant Natural := Within mod 8;
      ...
      Word_At : constant Interfaces.Unsigned_64 :=
        At_Block + 66 + Interfaces.Unsigned_64 (4 * Sub);
      Word    : constant Interfaces.Unsigned_64 :=
        Interfaces.Unsigned_64 (Byte_At (Word_At))
        + 256 * Interfaces.Unsigned_64 (Byte_At (Word_At + 1))
        + 65536 * Interfaces.Unsigned_64 (Byte_At (Word_At + 2))
        + 16777216 * Interfaces.Unsigned_64 (Byte_At (Word_At + 3));
      Step    : constant Long_Float :=
        Decode_Half (Image, At_Block)
        * (0.5 + Long_Float (Word / 2 ** 28)) * 0.5;
      Signs   : constant Natural :=
        Natural (KSigns_IQ2XS_Ref (Natural ((Word / 2 ** (7 * L)) mod 128)));
      Which   : constant Natural :=
        Byte_At (At_Block + 2
                 + Interfaces.Unsigned_64 (8 * Sub + 2 * L + J / 4));
      Magnitude : constant Natural :=
        Natural ((Interfaces.Unsigned_64 (IQ3XXS_Grid_Ref (Which))
                  / 2 ** (8 * (J mod 4))) mod 256);
   begin
      return Step * Long_Float (Magnitude)
        * (if (Signs / 2 ** J) mod 2 = 1 then -1.0 else 1.0);
   end Decode_IQ3_XXS;

   --  An IQ1 grid byte, which is signed: minus one, nought or one.
   function Signed_Grid_Byte (Entry_Value : Interfaces.Unsigned_64; J : Natural)
     return Long_Float
   is (Long_Float (if Grid_Byte (Entry_Value, J) >= 128
                   then Grid_Byte (Entry_Value, J) - 256
                   else Grid_Byte (Entry_Value, J)));

   --  One element of an IQ1_S super-block: fifty bytes, the scale, thirty-two
   --  bytes of grid index low bits -- one a lane -- and eight sixteen-bit
   --  words, one a sub-block: three index bits a lane from the bottom, a
   --  three-bit scale s at twelve giving a step of d (2 s + 1), and in the top
   --  bit the sign of an eighth added to every grid value.
   function Decode_IQ1_S
     (Image : Model_Runner.Bytes.Byte_Array;
      Base  : Interfaces.Unsigned_64;
      Index : Natural) return Long_Float
   is
      At_Block : constant Interfaces.Unsigned_64 :=
        Base + Interfaces.Unsigned_64 (Index / 256) * 50;
      Within   : constant Natural := Index mod 256;
      Sub      : constant Natural := Within / 32;
      L        : constant Natural := (Within mod 32) / 8;
      J        : constant Natural := Within mod 8;
      ...
      High  : constant Natural :=
        Byte_At (At_Block + 34 + Interfaces.Unsigned_64 (2 * Sub))
        + 256 * Byte_At (At_Block + 35 + Interfaces.Unsigned_64 (2 * Sub));
      Grid_Index : constant Natural :=
        Byte_At (At_Block + 2 + Interfaces.Unsigned_64 (4 * Sub + L))
        + 256 * ((High / 2 ** (3 * L)) mod 8);
      Step  : constant Long_Float :=
        Decode_Half (Image, At_Block)
        * Long_Float (2 * ((High / 2 ** 12) mod 8) + 1);
      Shift : constant Long_Float := (if High >= 16#8000# then -0.125 else 0.125);
   begin
      return Step
        * (Signed_Grid_Byte (IQ1S_Grid_Ref (Grid_Index), J) + Shift);
   end Decode_IQ1_S;

   --  One element of an IQ1_M super-block: fifty-six bytes and no scale field
   --  -- thirty-two bytes of grid index low bits, sixteen bytes that give each
   --  lane three index bits and a shift sign in a nibble, the first lane of a
   --  pair lowest, and four sixteen-bit scale words holding two sub-blocks'
   --  two three-bit scales apiece in their low twelve bits and, four bits a
   --  word from the top, the block's half-precision scale.
   function Decode_IQ1_M
     (Image : Model_Runner.Bytes.Byte_Array;
      Base  : Interfaces.Unsigned_64;
      Index : Natural) return Long_Float
   is
      At_Block : constant Interfaces.Unsigned_64 :=
        Base + Interfaces.Unsigned_64 (Index / 256) * 56;
      Within   : constant Natural := Index mod 256;
      Sub      : constant Natural := Within / 32;
      L        : constant Natural := (Within mod 32) / 8;
      J        : constant Natural := Within mod 8;
      ...
      function Scale_Word (W : Natural) return Natural
      is (Byte_At (At_Block + 48 + Interfaces.Unsigned_64 (2 * W))
          + 256 * Byte_At (At_Block + 49 + Interfaces.Unsigned_64 (2 * W)));

      D : constant Long_Float :=
        Half_Of (Scale_Word (0) / 2 ** 12
                 + (Scale_Word (1) / 2 ** 12) * 2 ** 4
                 + (Scale_Word (2) / 2 ** 12) * 2 ** 8
                 + (Scale_Word (3) / 2 ** 12) * 2 ** 12);

      Scale : constant Natural :=
        (Scale_Word (Sub / 2) / 2 ** (6 * (Sub mod 2) + 3 * (L / 2))) mod 8;
      Nibble : constant Natural :=
        (Byte_At (At_Block + 32 + Interfaces.Unsigned_64 (2 * Sub + L / 2))
         / 16 ** (L mod 2)) mod 16;
      Grid_Index : constant Natural :=
        Byte_At (At_Block + Interfaces.Unsigned_64 (4 * Sub + L))
        + 256 * (Nibble mod 8);
      Shift : constant Long_Float := (if Nibble >= 8 then -0.125 else 0.125);
   begin
      return D * Long_Float (2 * Scale + 1)
        * (Signed_Grid_Byte (IQ1S_Grid_Ref (Grid_Index), J) + Shift);
   end Decode_IQ1_M;
         ...
         if Index = 0
           or else Containers.Tensor_Format (Source, Index)
                   not in Model_Runner.GGUF.Type_F32
                        ...
                        | Model_Runner.GGUF.Type_IQ2_XS
                        | Model_Runner.GGUF.Type_IQ2_S
                        | Model_Runner.GGUF.Type_IQ3_XXS
                        | Model_Runner.GGUF.Type_IQ1_S
                        | Model_Runner.GGUF.Type_IQ1_M

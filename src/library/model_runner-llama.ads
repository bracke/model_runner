...
private
   ...
   type Session is limited new Ada.Finalization.Limited_Controlled with record
      ...
      --  new to a device holding sixteen busy sessions is warmer than any
      --  of them and takes a block from whichever has gone longest
      --  unasked, and set again at every ask after that.
      --
      --  What they are for is the other half of turning a session out: a
      --  session takes a block only from one that has gone unasked since
      --  before its own last ask. Seventeen sessions all reading a token
      --  apiece in turn would otherwise turn each other out every token
      --  and pay a cache written across the bus for every one of them,
      --  where the seventeenth doing without costs one session its speed
      --  and no more.
      --  A session asks once a layer and not once a token, so what it
      --  asked before this call is no measure of how warm it was: the
      --  layer before was this same session, a tick ago. Before is the
      --  stamp from the last run of asks somebody else's ask interrupted,
      --  which is the session's previous token, and that is what the
      --  guard reads.
      Asked_At           : Natural := 0;
      Asked_Before       : Natural := 0;
      State_Asked_At     : Natural := 0;
      State_Asked_Before : Natural := 0;

...
package body Model_Runner.CLI.Pictures is
   ...
   begin
      ...
      if Model_Runner.Vision.Placed_Rows (Item.Eyes) then
         ...
         Item.Bridge := Model_Runner.Text.Empty;
         Item.Gap := Model_Runner.Text.Empty;
         --  The slices' frame. 2.6 and later (version 3 and up) wrap each
         --  slice on its own, <slice> ... </slice> a slice, a line break
         --  after each grid row. 2.5 (version 2) wraps them another way:
         --  one <slice> ... </slice> round them all, each slice its own
         --  <image> ... </image> -- the picture marker -- a line break
         --  after each grid row. Where the file has the slice tokens; else
         --  the overview stands alone, as a small picture does.
         if Model_Runner.Tokenizer.Find (Words.all, "<slice>")
              /= Model_Runner.Tokenizer.No_Token
           and then Model_Runner.Tokenizer.Find (Words.all, "</slice>")
              /= Model_Runner.Tokenizer.No_Token
         then
            if Model_Runner.Vision.Minicpm_Version (Item.Eyes) >= 3 then
               ...
               Item.Has_Slices := True;
            else
               --  Version 2: the group round the slices, each slice the
               --  picture marker.
               Item.Slice_Group_Open :=
                 Model_Runner.Text.To_Bounded ("<slice>");
               Item.Slice_Group_Close :=
                 Model_Runner.Text.To_Bounded ("</slice>");
               Item.Slice_Marker_Text := Item.Marker_Text;
               Item.Slice_Row_End :=
                 Model_Runner.Text.To_Bounded ([1 => ASCII.LF]);
               Item.Has_Slices := True;
            end if;
   ...
   begin
      ...
      Into.Slice_Group_Open := Item.Slice_Group_Open;
      Into.Slice_Group_Close := Item.Slice_Group_Close;

...
- **A packed context stays packed on the device.** A `--kv-cache q8` or
  ...
  exact kernel's are; and a packed session's layer goes over whole, as
  the exact one's does -- normalized, projected, turned, packed, attended
  and fed in one submission, chained to the next layer's. The packing is
  a step of it, `pack.comp`, which rounds as the host rounds: the same
  scale, the same level, the same byte, every division taken through a
  double because a device's single-precision quotient need not be exact
  and the host's is; the host's copy of the block is read back out of
  the device's once a token or a batch is done, and a row the host packs
  itself is the same bytes, which the suite holds to the bit. A context
  shift moves the rows and puts them again. Held to the exact kernel
...
- **`http_get` and `web_search` fetch through an Ada HTTP client, not curl.**
  ...
  file, never held whole in memory, then returned capped. `web_search`
  percent-encodes its query into a GET to the DuckDuckGo lite endpoint.
  HTTPS is handled by the client's own TLS, so a machine without curl still
  fetches, and the client's default timeouts bound a slow or silent server
  the way the spawn watchdog bounded curl. `Max_Download_Size` holds the
  temporary file to no more than the tool will hand back. The crate was
  already in the build graph (`i18n`'s build tooling pulls it); this makes
  the library compile against it and adds `httpclient` as a direct
  dependency. `shell`, `run_python` and `sql` still spawn their programs.
...
- **An agentic loop, closing the tool loop this program left open.** The
  ...
  the model answers, a step budget runs out, or a wall-clock budget does --
  the clock is checked between steps, a generation in flight being bounded by
  its own token budget. A call the model has already

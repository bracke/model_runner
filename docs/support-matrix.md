...
| `embed` | Implemented: prints the hidden state of a text, pooled over its positions with `--pooling mean` or `--pooling last`, at unit length unless `--no-normalize`. One component a line. The prompt is read as written; no chat template is applied and none would be right. Evaluated a token at a time, because every position's state is wanted |

...
| `embed` | Implemented: prints the hidden state of a text, pooled over its positions with `--pooling mean` or `--pooling last`, at unit length unless `--no-normalize`. One component a line. The prompt is read as written; no chat template is applied and none would be right. Evaluated in batches, as a prompt is, with every position's state asked of the batched path; `--batch-size` sets how many go through the weights at once and does not change the answer |

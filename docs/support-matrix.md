...
| Repacking | both | Implemented, `--repack`: every weight matrix decoded once into binary32, the same logits to the bit, four bytes a weight against about one. Measured slower than the stored layout for Q8_0 and Q4_K and twelve per cent faster for Q2_K, so it is a flag and not a default |
